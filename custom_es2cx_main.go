...
var v3Agent *string = flag.String("cx-agent-id", "", "CX agent id")
var outFile *string = flag.String("out-file", "", "Output file for CSV TODO items")
var outFileIntentCorpus *string = flag.String("out-file-intent-corpus", "", "Output file for CSV intent corpus")
...
// intentCorpus for the CSV output
/*var intentCorpus = [][]string{
	{"IntentName", "IntentTFText", "IntentEntityType"},
}*/

var intentCorpus = [][]string{
	{"IntentName", "IntentTFFull", "IntentTFParts", "IntentTFEntity"},
}

// logIntentCorpus logs intent data for the CSV output
func logIntentCorpus(intentName string, intentTFFull []string, intentTFParts []string, intentTFEntity []string) {
	if len(intentTFParts) != 0 {
		intentCorpus = append(intentCorpus, []string{intentName, strings.Join(intentTFFull, " "), "[" + strings.Join(intentTFParts, ", ") + "]", "[" + strings.Join(intentTFEntity, ", ") + "]"})
	} else {
		intentCorpus = append(intentCorpus, []string{intentName, strings.Join(intentTFFull, " "), strings.Join(intentTFParts, ", "), strings.Join(intentTFEntity, ", ")})
	}
}
...
func convertIntent(intent2 *proto2.Intent) *proto3.Intent {
	...
	for _, p2 := range intent2.Parameters {
		...
		//fmt.Printf("params ->%s::::%s\n", p2.DisplayName, p2.EntityTypeDisplayName)
	...
	for _, tp2 := range intent2.TrainingPhrases {
		...
		var intentCorpusTFFull []string
		var intentCorpusTFParts []string
		var intentCorpusTFEntities []string
		for _, part2 := range tp2.Parts {
			...
			/*
				sort.Slice(parameters3, func(i, j int) bool {
					return parameters3[i].Id < parameters3[j].Id
				})*/
			if len(strings.TrimSpace(part2.Alias)) != 0 {
				sort.Slice(parameters3, func(i, j int) bool {
					return parameters3[i].Id < parameters3[j].Id
				})
				//fmt.Printf("Converted: %s\n", convertParameterEntityType(intent2.DisplayName, part2.Alias, part2.))
				i := sort.Search(len(parameters3), func(i int) bool { return parameters3[i].Id >= part2.Alias })
				if i >= len(parameters3) || parameters3[i].Id != part2.Alias {
					//fmt.Printf("Missing Params in intent: %s\n", intent2.DisplayName)
					parameters3 = append(parameters3, &proto3.Intent_Parameter{
						Id:         part2.Alias,
						EntityType: convertParameterEntityType(intent2.DisplayName, part2.Alias, part2.EntityType),
						IsList:     part2.UserDefined,
					})
				}
			}
			intentCorpusTFFull = append(intentCorpusTFFull, part2.Text)
			if len(strings.TrimSpace(part2.EntityType)) != 0 {
				intentCorpusTFParts = append(intentCorpusTFParts, part2.Text)
				intentCorpusTFEntities = append(intentCorpusTFEntities, part2.EntityType)
			}
			//logIntentCorpus(intent2.DisplayName, part2.Text, part2.EntityType)
		}
		logIntentCorpus(intent2.DisplayName, intentCorpusTFFull, intentCorpusTFParts, intentCorpusTFEntities)
...
func main() {
	...
	//logIntentCorpus
	csvFileIntentCorpus, err := os.Create(*outFileIntentCorpus)
	if err != nil {
		fmt.Fprintf(os.Stderr, "ERROR opening output file: %v", err)
		os.Exit(1)
	}
	defer closeFile(csvFileIntentCorpus)
	csvWriterIntentCorpus := csv.NewWriter(csvFileIntentCorpus)
	if err := csvWriterIntentCorpus.WriteAll(intentCorpus); err != nil {
		fmt.Fprintf(os.Stderr, "ERROR writing CSV output file: %v", err)
		os.Exit(1)
	}
	csvWriterIntentCorpus.Flush()
}
